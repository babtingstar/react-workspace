...
// 메인으로 확인할 배열리스트 () 내부에 들어올 변수명이 단일이기 때문에 users 에서 {} 틀 사용 여부는 상관 X
    ...
    return (
        ...
        <div>
            ...
            <ul>
                ...
                Parent 에서 가져온 변수이름.map(반복문 내에서 사용할 변수이름, 번호순서 = index) => ( <li key={index}> {user.name} / {user.age} */}
                {/*
                2. map{} -> return 반드시 사용해야함 / UI 와 기능을 동시에 표현할 때 많이 사용
                Parent 에서 가져온 변수이름.map{반복문 내에서 사용할 변수이름, 번호순서 = index) => ( <li key={index}> {user.name} / {user.age}} */}

                {/* html 태그 내에서 javascript 로 가져온 값을 사용하기 위해서는 반드시 {} 를 작성 후 변수 이름이나 기능 작성*/}
                {
...
const Parent = () => {
    // javascript 작성하는 공간
    // name 과 age 를 담을 배열 생성 -> 춯 데이터 베이스로 가져온 값 변형

    // 배열 형태는 [] 로 작성
    const numberList = [1, 2, 3]
    const stringList = ["가", "나", "다"]

    // 만약 배열에서 한 칸 당 값을 두 개 씩 가져오고 싶다면 {} 로 감싸서 가져오기
    const numberStringListOne = [
        {number: 1, string: "가"},
        {number: 2, string: "나"},
        {number: 1, string: "다"}
    ]
